...
[,]⇒prefix : ∀ (α β : V *) → length α ≤ length β → [ α , β ] → ∃[ η ] [ α , η ] × α ++ η ≡ β
[,]⇒prefix {V = V} α β ∣α∣≤∣β∣ [α,β] = η , [α,η] , αη≡β
  where
    η : V *
    η = take (length β ∸ length α) β

    αη≡β : α ++ η ≡ β
    αη≡β = begin
      α ++ η                   ≡˘⟨ take-++-≤ α β (length β) ∣α∣≤∣β∣ ⟩
      take (length β) (α ++ β) ≡⟨ cong (take (length β)) [α,β] ⟩
      take (length β) (β ++ α) ≡⟨ take-++-= β α ⟩
      β ∎
      where open ≡-Reasoning

    [α,αη] : [ α , α ++ η ]
    [α,αη] = subst [ α ,_] (sym αη≡β) [α,β]
...
module _ (V : Set a) (_≟_ : DecidableEquality V) where
  ...
  module _ (α : V *) (α≢ε : α ≢ [] ) where
    ...
    generator : V *
    generator = lookup αs (First.index αs-first)

    generator≢ε : generator ≢ []
    generator≢ε = inits′-≢[] α α≢ε (First.index αs-first) 
    
    generator-[,] : [ generator , α ]
    generator-[,] = [,]-sym α generator (First.index-satisfied αs-first)
    
    generator-min : ∀ δ → length δ < length generator → [ generator , δ ] → δ ≡ []
